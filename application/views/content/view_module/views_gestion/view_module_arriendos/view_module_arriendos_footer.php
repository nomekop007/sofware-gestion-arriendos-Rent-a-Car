</div>
<br><br>
</main>

</div>
</div>



<script>
$("#m_arriendo").addClass("active");
$("#l_arriendo").addClass("card");
</script>

<!-- importaciones del select2 -->
<script src="<?php echo base_route() ?>assets/js/select2.min.js"></script>

<!-- importaciones del arriendos -->
<script
    src="<?php echo base_route() ?>assets/js/js_gestion/js_module_arriendos/js_module_arriendos_registrar.js?v=<?php echo version(); ?>">
</script>

<script
    src="<?php echo base_route() ?>assets/js/js_gestion/js_module_arriendos/js_module_arriendos_todos.js?v=<?php echo version(); ?>">
</script>

<script src="<?php echo base_route() ?>assets/js/canvasVisorPDF.js?v=<?php echo version(); ?>"></script>

<script src="<?php echo base_route() ?>assets/js/canvasDosFirmasDigitales.js?v=<?php echo version(); ?>"></script>

<script src="<?php echo base_route() ?>assets/js/comunaCiudad.js?v=<?php echo version(); ?>"></script>